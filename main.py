...
# ✅ FIX : Mode sans écran (résout l'erreur libxcb.so.1 sur Replit/serveurs sans GUI)
...
# ==============================================================================
# 🧠 MOTEUR DE RECHERCHE AURA (SIMPLIFIÉ - SANS IA LOURDE)
# ==============================================================================
...
class MoteurRechercheAura:
    ...
    def indexer_tout_le_catalogue(self):
        """Simulation légère pour garder la structure intacte sans charger de modèles"""
        print("📊 [IA] Mode IA désactivé au démarrage pour économiser la RAM.")
        return

    def recherche_intelligente(self, query_text=None, query_image_file=None, top_k=20):
        """Retourne une liste vide pour éviter les plantages sans l'IA lourde"""
        return []
...
# --- LOGIQUE INTELLIGENTE GITHUB (CDN JSDELIVR) ---

def rendre_depot_public(user, token, repo_nom):
    """Force le dépôt en mode public pour que le CDN puisse lire la vidéo"""
    try:
...
def trouver_nimporte_quel_depot(user, token):
    """Scan le compte et trouve un dépôt disponible"""
    try:
        headers = {"Authorization": f"token {token}", "Accept": "application/vnd.github.v3+json"}
...
def upload_to_github(file_storage):
    """Envoie la vidéo et retourne un lien CDN jsDelivr"""
    try:
        comptes = [
        ...
        for c in comptes:
            ...
            if repo_nom:
                ...
                print(f"📦 Stockage fluide via CDN sur : {repo_nom} (Compte: {u})")
    ...
    try:
        ...
        if ids_produits:
            try:
                ...
                ids_marchands = list(set(
                    p['id_commercant'] for p in produits_map.values()
                    if p.get('id_commercant')
                ))
                if ids_marchands:
                ...
                logging.warning(f"[PANIER] Erreur chargement produits : {e_prod}")
        ...
        total_panier = sum(
            float(item.get('prix_unitaire_ajoute') or 0) * int(item.get('quantite') or 1)
            for item in items
        )
...
def api_ajouter_au_panier():
    ...
    try:
        ...
        if check_item.data:
            ...
            supabase.table('panier_items').insert({
                "id_panier": id_panier, "id_produit": prod_id, "quantite": 1, 
                "prix_unitaire_ajoute": prix, "taille": taille, "couleur": couleur
            }).execute()
...
def compte():
    ...
    try:
        ...
        logging.warning(f"[COMPTE] Profil absent pour user_id={user_id}")
...
def api_modifier_profil():
    ...
    try:
        ...
        logging.info(f"[PROFIL] ✅ Profil modifié pour {user_id}")
        return jsonify({"status": "success"})
    except Exception as e:
        logging.error(f"[PROFIL] ❌ Erreur : {e}")
        return jsonify({"error": str(e)}), 500
...
def get_product_details(prod_id):
    try:
        # ✅ CORRECTION : Ajout des colonnes couleurs et tailles dans le select
        res = supabase.table('produits').select(
            'id, nom, prix, ancien_prix, stock, stock_total, image_url, video_url, description, categorie, couleurs, tailles, '
            'profil!id_commercant(nom_boutique, photo_url), '
            'images_details(*), variantes(*)'
...
def delete_banner(pos):
    try: supabase.table('banners').delete().eq('position', pos).execute()
    except Exception as e: flash(f"Erreur : {e}", "error")
    return redirect(url_for('admin_dashboard'))
...
def inscription_commercant():
    ...
    try:
        ...
        if request.method == 'POST':
            nom_boutique, telephone, num_cni, adresse_boutique = request.form.get('nom_boutique'), request.form.get('telephone'), request.form.get('num_cni'), request.form.get('adresse_boutique')
            def upload_doc(file, folder):
                if file:
                    ext = os.path.splitext(file.filename)[1].lower()
                    filename = f"{folder}/{user_id}_{uuid.uuid4()}{ext}"
                    supabase.storage.from_('verifications').upload(filename, file.read())
                    return supabase.storage.from_('verifications').get_public_url(filename)
                return None
            supabase.table('profil').update({
                "role": "en_attente_validation", "nom_boutique": nom_boutique, "telephone": telephone,
                "num_cni": num_cni, "adresse_boutique": adresse_boutique,
            ...
            flash("Documents envoyés !", "success"); return redirect(url_for('compte'))
...
def merchant_dashboard():
    ...
    try:
        ...
        commandes_reelles = []
        for o in res_orders:
            nom_c = f"{o['profil']['nom']} {o['profil']['prenom']}" if o.get('profil') else "Client"
            commandes_reelles.append({"nom_client": nom_c, "heure_commande": o.get('heure_commande', 'N/A'), "telephone_client": o.get('telephone_client', 'Aucun'), "nom_produit": o.get('nom_produit', 'Article'), "prix_total": o.get('prix_total', 0), "statut": o.get('statut', 'en attente')})
        return render_template('admin_commercant.html', produits=produits, total_ca=sum(o.get('prix_total', 0) for o in res_orders if o.get('statut') == 'livré'), non_livres_count=len([o for o in res_orders if o.get('statut') != 'livré']), stock_restant=sum(int(p.get('stock', 0)) for p in produits), total_produits_count=len(produits), total_vendus_count=len([o for o in res_orders if o.get('statut') == 'livré']), capital_stock=sum((int(p.get('prix', 0)) * int(p.get('stock', 0))) for p in produits), commandes_reelles=commandes_reelles, categories=CATEGORIES_LIST)
    except Exception as e: return render_template('admin_commercant.html', produits=[], total_ca=0, categories=CATEGORIES_LIST)
...
def add_product():
    try:
        ...
        if file and file.filename != '':
            image_url = cloudinary.uploader.upload(designer_automatique_ia(file)).get('secure_url')
        v_file = request.files.get('video_produit')
        if v_file and v_file.filename != '':
            video_url = upload_to_github(v_file)

        # ✅ CORRECTION : Ajout des colonnes couleurs et tailles au format JSON lors de l'insertion
        tailles, couleurs = request.form.getlist('tailles[]'), request.form.getlist('couleurs[]')
        
        data_produit = {
            "id_commercant": session.get('user_id'), "nom": request.form.get('nom'), "description": request.form.get('description'),
            "prix": int(float(request.form.get('prix', 0))), "ancien_prix": int(float(request.form.get('ancien_prix', 0))) if request.form.get('ancien_prix') else None,
            "stock": int(request.form.get('stock', 0)), "stock_total": int(request.form.get('stock_total', 0)) or int(request.form.get('stock', 0)),
            "categorie": request.form.get('categorie'), "etat_produit": request.form.get('etat_produit'), "image_url": image_url, "video_url": video_url,
            "couleurs": json.dumps(couleurs), "tailles": json.dumps(tailles)
        }
        ...
        if res.data:
            ...
            if not tailles and not couleurs: supabase.table('variantes').insert({"id_produit": new_id, "taille": None, "couleur": None, "stock_variante": data_produit['stock']}).execute()
            else:
            ...
            for d_file in request.files.getlist('images_details'):
                if d_file and d_file.filename != '':
                    supabase.table('images_details').insert({"id_produit": new_id, "image_url": cloudinary.uploader.upload(designer_automatique_ia(d_file)).get('secure_url')}).execute()
            ...
            threading.Thread(
                target=_broadcaster_tous,
                args=("🆕 Nouveau produit !", msg, "nouveau_produit"),
                daemon=True
            ).start()

            threading.Thread(
                target=_push_broadcast_tous,
                args=("🆕 Nouveau produit !", msg, "/", "nouveau_produit"),
                daemon=True
            ).start()
...
def edit_product(prod_id):
    ...
    if request.method == 'POST':
        try:
            ...
            updated_data = {"nom": request.form.get('nom'), "prix": p_nouveau, "ancien_prix": int(produit_actuel['prix']) if p_nouveau < produit_actuel['prix'] else produit_actuel['ancien_prix'], "stock": int(request.form.get('stock', 0)), "description": request.form.get('description'), "categorie": request.form.get('categorie')}
            ...
            if p_nouveau < p_ancien and p_ancien > 0:
                ...
                msg_promo = (
                    f"🎉 Promo -{reduction}% sur {nom_p} !\n"
                    f"Ancien prix : {p_ancien:,} FCFA → Nouveau : {p_nouveau:,} FCFA"
                ).replace(",", " ")
                threading.Thread(
                    target=_broadcaster_tous,
                    args=(f"🔥 -{reduction}% sur {nom_p} !", msg_promo, "promo"),
                    daemon=True
                ).start()
                threading.Thread(
                    target=_push_broadcast_tous,
            ...
            flash("Mis à jour !", "success"); return redirect(url_for('merchant_dashboard'))
        except Exception as e: flash(f"Erreur : {e}", "error")
...
def galerie_videos():
    ...
    try:
        if query:
            ids_trouves = moteur_ia.recherche_intelligente(query_text=query)
            if ids_trouves:
                res = supabase.table('produits').select('*, profil!id_commercant(nom_boutique, photo_url)') \
                    .in_('id', ids_trouves) \
...
def api_videos_recherche():
    ...
    try:
        ids_trouves = moteur_ia.recherche_intelligente(query_text=query)
        if ids_trouves:
            res = supabase.table('produits') \
                .select('id, nom, prix, video_url, image_url, likes_count') \
                .in_('id', ids_trouves) \
                .not_.is_('video_url', 'null') \
                .execute()
            produits_trouves = res.data or []
        ...
        logging.error(f"Erreur API recherche vidéo: {e}")
        return jsonify([]), 500
...
def handle_comments(prod_id):
    if request.method == 'POST':
        ...
        try:
            supabase.table('commentaire_produit').insert({
                "produit_id": prod_id, 
                "user_id": user_id, 
                "commentaire_texte": content
            }).execute()
...
def api_commander_panier():
    ...
    try:
        res_client = supabase.table('profil') \
        ...
        if ids_marchands:
            res_m = supabase.table('profil') \
                .select('id, nom_boutique, telephone, email, callmebot_key') \
                .in_('id', ids_marchands).execute()
        ...
        for cid, groupe in groupes.items():
            ...
            contenu_marchand = (
                f"Vous avez reçu {nb_articles} article(s) commandé(s) par {nom_client} !\n\n"
                + "\n".join(lignes_marchand)
                + f"\n\n💰 Sous-total : {int(sous_total):,} FCFA".replace(",", " ")
                + f"\n👤 Client : {nom_client}"
            ...
            if cid:
                supabase.table('messages').insert({
                    "receiver_id":      cid,
                    "expediteur":       f"Commande de {nom_client}",
                    "contenu":          contenu_marchand,
                    "type":             "commande",
                    "telephone_client": tel_client,
                    "lu":               False
                }).execute()
            ...
            if cid:
                ...
                if nb_articles > 3:
                    corps_push += f" +{nb_articles-3} autre(s)"
                corps_push += f" — {int(sous_total):,} FCFA total".replace(",", " ")

                threading.Thread(
                    target=_notifier_push_utilisateur,
                    args=(cid, titre_push, corps_push, "/compte", "commande"),
                    daemon=True
                ).start()

            threading.Thread(
                target=_notifier_en_arriere_plan,
                args=(
                    {
                        "nom_produit": f"{nb_articles} article(s)",
                        "quantite": sum(i.get('quantite',1) for i in articles),
                        "prix_total": sous_total,
                        "taille": None, "couleur": None,
                        "telephone_client": tel_client
                    },
                    marchand,
                    client
        ...
        nb_total = len(items)
        contenu_client = (
            f"Votre commande de {nb_total} article(s) a bien été enregistrée !\n\n"
            + "\n".join(liste_produits_client)
            + f"\n\n💰 Total : {int(total_global):,} FCFA".replace(",", " ")
            + "\n\n⏳ Les commerçants vont vous contacter pour la livraison."
        )
        supabase.table('messages').insert({
            "receiver_id": user_id,
            "expediteur":  "Aura Markeplay",
        ...
        threading.Thread(
            ...
            args=(
                user_id,
                f"✅ Commande de {nb_total} article(s) confirmée !",
                f"Total : {int(total_global):,} FCFA — Les marchands vont vous contacter.".replace(",", " "),
                "/compte",
                "confirmation"
            ),
            daemon=True
        ...
        try:
            res_p = supabase.table('panier') \
                .select('id_panier').eq('id_user', user_id).eq('statut', 'actif').execute()
            if res_p.data:
            ...
            logging.warning(f"[PANIER] Erreur vidage panier : {ep}")
...
def _whatsapp_callmebot(telephone: str, apikey: str, message: str):
    try:
        ...
        requests.get(
            ...
            params={"phone": tel, "text": message, "apikey": apikey},
            timeout=20
        )
        logging.info(f"[WHATSAPP] Message CallMeBot envoyé au {tel}")
    except Exception as e:
        logging.error(f"[WHATSAPP] Erreur CallMeBot : {e}")
...
def _notifier_en_arriere_plan(commande: dict, marchand: dict, client: dict):
    def _run():
        ...
        if email_marchand:
            details = f"""
            <li>📦 Produit : <strong>{nom_prod}</strong></li>
            <li>🔢 Quantité : <strong>{qty}</strong></li>
            <li>💰 Total : <strong>{int(total):,} FCFA</strong></li>
            <li>👤 Client : <strong>{nom_client}</strong></li>
            <li>📱 Téléphone : <strong>{tel_client}</strong></li>
            """.replace(",", " ")
            if commande.get("taille"):
                details += f"<li>📐 Taille : <strong>{commande['taille']}</strong></li>"
            if commande.get("couleur"):
                details += f"<li>🎨 Couleur : <strong>{commande['couleur']}</strong></li>"
            ...
            <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;
              ...
              <div style="padding:24px;">
                <p style="font-size:15px;font-weight:700;color:#0f172a;margin-bottom:16px;">
                  Bonjour {nom_boutique}, vous avez une nouvelle commande !
                </p>
                <div style="background:#f8fafc;border-radius:12px;padding:16px;margin-bottom:16px;">
                  <ul style="margin:0;padding-left:18px;line-height:2.2;color:#0f172a;font-size:14px;">
                    {details}
                  </ul>
                </div>
                <p style="font-size:12px;color:#64748b;text-align:center;">
                  <a href="https://aurafr.pythonanywhere.com/merchant/dashboard"
            ...
            if admin:
                _envoyer_email_bg(admin, f"[COPIE] Commande {nom_prod} ×{qty}", html)

        tel_marchand  = marchand.get("telephone", "")
        callmebot_key = marchand.get("callmebot_key", "")
        if tel_marchand and callmebot_key:
            msg_wa = (
                f"🛍️ *NOUVELLE COMMANDE — Aura Markeplay*\n\n"
                f"📦 *Produit :* {nom_prod}\n"
                f"🔢 *Quantité :* {qty}\n"
                f"💰 *Total :* {int(total):,} FCFA\n"
                f"👤 *Client :* {nom_client}\n"
                f"📱 *Téléphone :* {tel_client}\n\n"
                f"✅ Contactez le client pour la livraison !"
            ).replace(",", " ")
...
def api_commander():
    ...
    try:
        ...
        contenu_marchand = (
            ...
            f"🔢 Quantité : {quantite}\n"
            f"💰 Total : {int(prix_total):,} FCFA{variantes}\n\n"
            f"👤 Client : {nom_client}\n"
            f"📱 Téléphone : {tel_client}"
        ).replace(",", " ")
        ...
        contenu_client = (
            ...
            f"Le commerçant va vous contacter pour la livraison."
        ).replace(",", " ")

        if commercant_id:
            supabase.table('messages').insert({
                "receiver_id":      commercant_id,
                "expediteur":       f"Commande de {nom_client}" if nom_client else "Nouvelle commande",
                "contenu":          contenu_marchand,
                "type":             "commande",
                "telephone_client": tel_client or "",
                "lu":               False
            }).execute()

        supabase.table('messages').insert({
            "receiver_id": user_id,
            "expediteur":  nom_boutique or "Aura Markeplay",
            "contenu":     contenu_client,
            "type":        "confirmation",
            "lu":          False
        }).execute()
        ...
        if commercant_id:
            threading.Thread(
                ...
                args=(
                    ...
                    f"{nom_client} a commandé : {nom_produit} ×{quantite}",
                    "/compte",
                    "commande"
                ),
                daemon=True
        ...
        threading.Thread(
            ...
            args=(
                ...
                f"Votre commande {nom_produit} a bien été enregistrée.",
                "/compte",
                "confirmation"
            ),
            daemon=True
...
def _preparer_cle_vapid_privee(raw: str) -> str:
    ...
    try:
        ...
        logging.info(f"[PUSH] 🔑 Clé base64url OK ({len(cle_nette)} chars)")
        return cle_nette
    except Exception as e:
...
def _envoyer_push(abonnement: dict, titre: str, corps: str,
                  url: str = '/compte', type_notif: str = 'defaut',
                  badge_count: int = 0):
    if not PUSH_ACTIF:
        return
    ...
    try:
        ...
        try:
            if hasattr(e, 'response') and e.response is not None:
                ...
                logging.error(f"[PUSH] ❌ HTTP {status} : {e.response.text[:300]}")
                if status in (404, 410):
                    ep = abonnement.get('endpoint', '')
                    if ep:
                        supabase.table('push_abonnements').delete().eq('endpoint', ep).execute()
                        logging.info(f"[PUSH] 🗑️ Abonnement expiré supprimé")
        except:
            pass
    except Exception as e:
        logging.error(f"[PUSH] ❌ {type(e).__name__} → {e}")

def _notifier_push_utilisateur(user_id: str, titre: str, corps: str,
                                url: str = '/compte', type_notif: str = 'defaut'):
    try:
        res = supabase.table('push_abonnements').select('abonnement').eq('user_id', user_id).execute()
        abonnements = res.data or []
        if not abonnements:
            logging.info(f"[PUSH] ℹ️ Aucun abonnement pour {user_id}")
            return
        badge_count = _compter_messages_non_lus(user_id)
        logging.info(f"[PUSH] 📤 Privé → {len(abonnements)} appareil(s), badge={badge_count}")
        for row in abonnements:
            try:
                abo = row['abonnement']
                if isinstance(abo, str):
                    abo = json.loads(abo)
                if not abo.get('endpoint'):
                    continue
                threading.Thread(
                    target=_envoyer_push,
                    args=(abo, titre, corps, url, type_notif, badge_count),
                    daemon=True
                ).start()
            except Exception as e:
...
def _push_broadcast_tous(titre: str, corps: str, url: str = '/',
                          type_notif: str = 'nouveau_produit'):
    try:
        res = supabase.table('push_abonnements').select('user_id, abonnement').execute()
        abonnements = res.data or []
        if not abonnements:
            logging.info("[PUSH BROADCAST] Aucun abonné")
            return
        logging.info(f"[PUSH BROADCAST] 📢 '{titre}' → {len(abonnements)} appareils")
        for row in abonnements:
            try:
                abo = row['abonnement']
                if isinstance(abo, str):
                ...
                threading.Thread(
                    target=_envoyer_push,
                    args=(abo, titre, corps, url, type_notif, 0),
                    daemon=True
                ).start()
            except Exception as e:
                logging.warning(f"[PUSH BROADCAST] ⚠️ Abonnement invalide : {e}")
    except Exception as e:
...
def api_push_abonner():
    ...
    try:
        ...
        if not res.data:
            supabase.table('push_abonnements').insert({
                "user_id":    user_id,
                "abonnement": json.dumps(abonnement),
                "endpoint":   endpoint
            }).execute()
            logging.info(f"[PUSH] ✅ Abonnement enregistré pour {user_id}")
        return jsonify({"status": "ok"})
    except Exception as e:
        logging.error(f"[PUSH] Erreur sauvegarde abonnement : {e}")
        return jsonify({"error": str(e)}), 500
...
def api_push_debug_son():
    ...
    resultats = []
    resultats.append(("pywebpush installé",    PUSH_ACTIF,
        "OK" if PUSH_ACTIF else "❌ pip install pywebpush"))
    resultats.append(("VAPID_PUBLIC_KEY",       bool(cle_pub),
        f"{len(cle_pub)} chars" if cle_pub else "❌ ABSENT dans Secrets"))
    resultats.append(("VAPID_PRIVATE_KEY",      bool(cle_priv),
        f"{len(cle_priv)} chars" if cle_priv else "❌ ABSENT dans Secrets"))
    resultats.append(("Abonnements push",       len(abos) > 0,
        f"{len(abos)} appareil(s)" if abos else "❌ 0 — clique Autoriser sur l'app"))

    sw_path = os.path.join(os.path.dirname(__file__), 'static', 'sw.js')
    sw_ok   = os.path.exists(sw_path)
    resultats.append(("static/sw.js présent",   sw_ok,
        f"{os.path.getsize(sw_path)} octets" if sw_ok else "❌ FICHIER MANQUANT"))

    envoi_ok, envoi_msg = False, "Non testé (il manque quelque chose ci-dessus)"
    if PUSH_ACTIF and cle_pub and cle_priv and abos:
        try:
            abo = abos[0]['abonnement']
            if isinstance(abo, str):
                abo = json.loads(abo)
            _envoyer_push(abo, "🔊 Test Son — Aura",
                "Si tu entends ce son, le son fonctionne parfaitement !",
                "/compte", "commande", 1)
            envoi_ok  = True
            envoi_msg = "✅ Notification envoyée — regarde ton téléphone et écoute !"
        except Exception as e:
            envoi_msg = f"❌ Erreur envoi : {e}"
    resultats.append(("Envoi notification test", envoi_ok, envoi_msg))

    raisons = [
        "🔕 Mode Ne pas déranger activé → désactive-le sur ton téléphone",
        "🔇 Volume des notifications à 0 → monte le volume sonnerie/notifications",
        "📱 Android : Paramètres → Applis → Chrome → Notifications → Son = ON",
        "🍎 iPhone Safari : le son push n'est PAS supporté (limitation Apple)",
        "💻 PC : Paramètres Windows/Mac → Notifications → Chrome → Son = ON",
        "🔄 SW pas rechargé : DevTools → Application → Unregister → Ctrl+Shift+R",
        "🌐 Le site doit être en HTTPS (pas HTTP) pour que le son fonctionne",
    ]

    lignes     = "".join(f'<tr><td>{n}</td><td style="color:{"#4ade80" if ok else "#f87171"}">{"✅" if ok else "❌"}</td><td>{d}</td></tr>' for n,ok,d in resultats)
    raisons_li = "".join(f"<li>{r}</li>" for r in raisons)

    return f"""<!DOCTYPE html>
<html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>🔊 Debug Son — Aura</title>
<style>
  body{{font-family:monospace;background:#0f172a;color:#e2e8f0;padding:20px;margin:0;font-size:13px}}
  h1{{color:#D4AF37;font-size:18px}}
  h2{{color:#94a3b8;font-size:12px;text-transform:uppercase;letter-spacing:2px;margin-top:24px}}
  table{{width:100%;border-collapse:collapse;background:#1e293b;border-radius:12px;overflow:hidden;margin-bottom:16px}}
  th{{background:#334155;padding:10px;text-align:left;font-size:11px;text-transform:uppercase;letter-spacing:1px}}
  td{{padding:10px;border-bottom:1px solid #334155}}
  ul{{background:#1e293b;border-radius:12px;padding:16px 16px 16px 32px;line-height:2.2;margin:0}}
  .btn{{display:inline-block;margin:12px 4px 0;padding:12px 20px;border-radius:10px;color:white;text-decoration:none;font-weight:bold;font-size:12px}}
</style></head><body>
<h1>🔊 Debug Son Notifications — Aura</h1>
<table><tr><th>Test</th><th>Statut</th><th>Détail</th></tr>{lignes}</table>
<h2>Raisons possibles si le son ne sort pas</h2>
<ul>{raisons_li}</ul>
<a href="/api/push/debug_son" class="btn" style="background:#2563eb">🔄 Retester + renvoyer</a>
<a href="/compte" class="btn" style="background:#16a34a">← Compte</a>
</body></html>"""
...
def _broadcaster_tous(titre: str, contenu: str, type_msg: str = "info"):
    try:
        ...
        users = res.data or []
        if not users:
            logging.warning("[BROADCAST] Aucun utilisateur trouvé")
            return 0
...
<title>📢 Notifier tous — Aura</title>
<style>
  body{font-family:sans-serif;background:#0f172a;color:#e2e8f0;
       padding:20px;margin:0;max-width:500px;margin:0 auto;}
  h1{color:#D4AF37;font-size:18px;margin-bottom:24px}
  ...
  #resultat{margin-top:16px;padding:14px;border-radius:10px;
            background:#1e293b;display:none}
  .ok{color:#4ade80} .err{color:#f87171}
</style>
</head>
<body>
<h1>📢 Envoyer message à TOUS les utilisateurs</h1>

<label>Type de message</label>
<select id="type">
  <option value="info">📢 Information générale</option>
  <option value="promo">🎉 Promotion / Réduction</option>
  <option value="nouveau_produit">🆕 Nouveau produit</option>
  <option value="confirmation">✅ Confirmation</option>
</select>

<label>Titre</label>
<input id="titre" placeholder="Ex: 🔥 Soldes -30% !" value="Aura Markeplay">

<label>Message</label>
<textarea id="contenu" placeholder="Ex: Profitez de -30% sur tous les articles jusqu'au 31 mars !"></textarea>

<button class="btn" onclick="envoyer()">📤 Envoyer à tous les utilisateurs</button>
...
async function envoyer() {
  ...
  const res_el  = document.getElementById('resultat');

  if (!contenu) { alert('Écris un message !'); return; }

  res_el.style.display = 'block';
  res_el.innerHTML = '<p>⏳ Envoi en cours...</p>';

  try {
    const res  = await fetch('/api/admin/notifier_tous', {
      method:  'POST',
    ...
    if (data.status === 'ok') {
      res_el.innerHTML = '<p class="ok">✅ Message envoyé à ' + data.envoye_a + ' utilisateurs !</p>';
      document.getElementById('contenu').value = '';
    } else {
      res_el.innerHTML = '<p class="err">❌ Erreur : ' + (data.error || '?') + '</p>';
    }
  } catch(e) {
    res_el.innerHTML = '<p class="err">❌ Erreur réseau</p>';
  }
}
</script>
</body>
