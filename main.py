...
def galerie_videos():
    ...
    try:
        if query:
            ids_trouves = moteur_ia.recherche_intelligente(query_text=query)
            if ids_trouves:
                res = supabase.table('produits').select(SELECT_VIDEOS).in_('id', ids_trouves).not_.is_('video_url', 'null').execute()
                liste_produits = res.data or []
            if not liste_produits:
                res = supabase.table('produits').select(SELECT_VIDEOS).ilike('nom', f"%{query}%").not_.is_('video_url', 'null').execute()
                liste_produits = res.data or []
        else:
            res = supabase.table('produits').select(SELECT_VIDEOS).not_.is_('video_url', 'null').execute()
            liste_produits = res.data or []
...
def handle_comments(prod_id):
    ...
    try:
        ...
        for c in res.data:
            ...
            avatar_url = c['profil'].get('photo_url', '') if c.get('profil') else ''
